...
class Deck:
   ...
   def add_slide(self, slide, index=None, layout=None):
      #  Check slide is a Slide object
      if (not isinstance(slide, Slide)):
         raise TypeError("--- can only add Slide objects ---")

      #  Determine appropriate layout using find_layout() method if user
      #  didn't specify the layout
      lolist = self.find_layout(slide)
      if (layout is not None):
         if (layout < len(self.pres.slide_layouts)):
            lo = layout
            if (lo not in lolist):
               print("---  specified layout is invalid; switching layout  ---")
               lo = lolist[0]
         else:
            raise IndexError("--- specified layout outside slide layout range ---")
      else:
         lo = lolist[0]
      print("---  using layout {}".format(lo))
   ...
   def render_slide(self, slide, layout=None, index=None):
      ...
      #  Fill the slide exhibit header (SectionHeader) placeholders, if applicable
      if (slide.num_exheaders() > 0):
         ph = self.get_exhibitheader_ph(lo)
         i = 0
         while (i < len(slide.exheaders)):
            new_slide.placeholders[ph.pop()].text = slide.exheaders[i]
            i += 1
      ...
      if (slide.num_main_bullets() > 0):
         ...
         #  Add first bullet directly, additional ones via paragraphs
         for i in range(3):
         ...
         #  Loop through and implement the formatting "runs" for remaining bullets
         for i in range(3, len(slide.run_main)):
            if (re.search("ParaLevel", slide.run_main[i])):
               (junk, level) = slide.run_main[i].split("Level")
      ...
      if (slide.num_margin_bullets() > 0):
         ...
         for i in range(3, len(slide.run_marg)):
            if (re.search("ParaLevel", slide.run_marg[i])):
               (junk, level) = slide.run_marg[i].split("Level")
   ...
   #  Return SectionHeader (exhibit header) placeholders in specified slide layout
   def get_exhibitheader_ph(self, i):
      ph = []
      for key in self.slide_los[i].keys():
         if (re.search("^SectionHeader", key)):
  ...
  #  Return Table placeholders in specified slide layout
   ...
   #  Return number of exhibit headers (SectionHeader) placeholders in specified slide layout
   def num_exheader_ph(self, i):
      p = 0
      for key in self.slide_los[i].keys():
         if (re.search("^SectionHeader", key)):
            p += 1
   ...
   #  Return number of exhibit (Picture) placeholders in specified slide layout
...
class Slide:
   def __init__(self, name, title=None, exhibits=None, bullets_main=None, bullets_marg=None, footnotes=None, exheaders=None):
      self.name = name
      self.title = title
      ...
      if (self.exheaders is not None):
         if (not isinstance(self.exheaders, list)):
            raise TypeError("--- exheaders must be a list ---")
         else:
            for h in self.exheaders:
               if (not isinstance(h, str)):
                  raise TypeError("--- exheaders must contain only strings ---")
   ...
   #  Return number of exhibit headers
   def num_exheaders(self):
      if (self.exheaders is not None):
         return len(self.exheaders)
      else:
         return 0
   ...
   #  Add exhibit headers
   def add_exheaders(self, header):
      #  Add exhibit header to the exheaders list if it is a string
      if (isinstance(header, str)):
         raise TypeError("--- exhibit headers must be strings ---")
      else:
         if (self.exheaders is None):
            self.exheaders = []
         self.exheaders.append(header)
         return True

   #  Get list of exhibit headers
   def get_exheaders(self):
      return self.exheaders
...
deck = Deck("c:/users/schmi/documents/python/slidedeck/tsgempty.pptx")
# deck.show_layouts()
slide1 = Slide("econoverview", exhibits=["bt.png", "pay.png", "sales.png", "cash.png"], exheaders=["Header 1", "Header 2", "Header 3", "Header 4"])
slide1.add_margin_bullets("- Here's a bullet...")
slide1.add_margin_bullets("- And another!")
slide1.add_title("U.S. Economy Continues to Expand")
# print("Placeholders==============================")
# deck.show_placeholders(32)
exhph = deck.num_exheader_ph(32)
picph = deck.num_picture_ph(32)
print("Exhibit header and picture placeholders = {}, {}".format(exhph, picph))
los = deck.find_layout(slide1)
print('Suggested slide layout:')
print(los)
slidenum = deck.add_slide(slide1, None, los[0])
deck.save('tsgtest.pptx')


# slidenum = deck.add_slide(slide1, None, 3)
# print('Slide title: {}'.format(slide1.title))
# deck.show_layouts()
# print('')
# los = deck.find_layout(slide1)
# print('Suggested slide layout:')
...
# if (los[0] != -999):
#    deck.show_placeholders(los[0])
#    slidenum = deck.add_slide(slide1, los[0])
#    print('Slide number: {}'.format(slidenum))
#    print('Slide name: {}'.format(deck.slides[0].name))


# slide1 = Slide("econoverview", exhibits=["pay.png", "sales.png", "cash.png"], bullets_marg=["This is the first bullet", "Here's the second bullet"])
# slide1 = Slide("econoverview", bullets_main=["First main bullet", "Second main bullet"])
# print('Slide title: {}'.format(slide1.title))
# slide1.add_title("U.S. Economy Continues to Expand")
# print('Slide title: {}'.format(slide1.title))
# numexhibits = slide1.num_exhibits()
# print('Number of exhibits: {}'.format(numexhibits))
# print('Slide exhibits: {}'.format(slide1.exhibits))
# print('')
# deck.show_layouts()
# print('')
# los = deck.find_layout(slide1)
